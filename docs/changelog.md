...
- 修复生成历史保存时间偏移问题：历史批次现在直接保存本机本地时间和时区，避免显示时间比系统时间慢约 9 小时。
- 修复历史记录在极短时间内连续写入时的排序不稳定问题：当多个批次时间戳相同，列表按实际保存顺序展示，缓存剪枝优先删除最早保存的批次。
- 修复 Anki 批量推送在 `create_only` 模式下未按批量创建路径执行的问题，提升大量卡片推送的一致性。
- 修复 `create_or_update` 和 `update_only` 模式下验证失败仍继续推送的风险，避免无效卡片进入后续 Anki 写入流程。
- 修复批量推送进度显示可能回退的问题，进度反馈现在保持单调递增。
- 修复卡片预览列表最多只展示 2000 条的限制，避免大批量生成结果在预览阶段被截断。

### 改进

- 历史页面展示带时区的历史时间时，会转换为当前本机时区后显示，保证保存值和界面显示一致。
- 明确日志等级过滤语义：`DEBUG` 包含全部日志，`INFO` 包含 `INFO`、`WARNING`、`ERROR` 等更高等级信息，并补充回归测试锁定行为。
- 完善导入、预览、结果和卡片预览页面的状态同步与回归测试，降低批量任务流程中的 UI 状态漂移。

### 验证

- 通过 `uv run ruff check src tests`。
- 通过 `QT_QPA_PLATFORM=offscreen uv run pytest tests --ignore=tests/e2e -q --maxfail=1`，共 `888 passed`。
- 通过 `uv run python packaging/build.py --smoke-test-only --version 0.3.3`。
- 已本地生成 Windows 便携版与安装版发布包。

## [v0.3.1] - 2026-06-08

### 变更

- ✅ 新增“历史结果”页面，可查看生成批次、来源文件、卡片数量，并支持批量删除与导出。
- ✅ 将生成历史迁移为安装目录下的 SQLite 缓存，并支持按缓存容量与记录数量自动清理最早记录。
- ✅ 设置页新增历史缓存上限配置，默认 `500 MB`、`500` 条记录，清空缓存时同步清理历史结果。
- ✅ 支持历史结果导出为 APKG、JSON、CSV 和 Markdown，为后续历史读取与导出功能打好基础。

## [v0.2.5] - 2026-03-24

### 变更

- ✅ 完善卡片增强链路，统一自动格式化流程，并补齐生成结果质量信号与回退路径。
- ✅ 收紧导入到生成阶段的任务反馈与通知展示，可恢复任务链路更稳定。
- ✅ 恢复应用读取系统主体色能力，并将主色同步到窗口与关键状态展示区域。

## [v0.2.1] - 2026-03-17

### 变更

- ✅ 删除导入页与预览页中的“最近耗时”提示区，页面信息层级更简洁。
- ✅ 调整安装器完成页为两个选项，仅保留“创建桌面快捷方式”和“启动应用”。
- ✅ 完成版本号升级到 `0.2.1`，统一运行时、项目元数据与安装器版本标识。

## [v0.2] - 2026-03-17

### 变更

- ✅ 优化应用启动链路，减少主窗口与导入页首屏构造时的同步开销。
- ✅ 调整导入页策略配置区为首帧后按需初始化，同时补齐相关回归测试。
- ✅ 完成版本号升级到 `0.2`，统一运行时、项目元数据与安装器版本标识。
