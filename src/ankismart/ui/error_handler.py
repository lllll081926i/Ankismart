...
class ErrorHandler:
    ...
    def _build_error_patterns(self) -> dict[str, ErrorInfo]:
        ...
        return {
            ...
            "connection": ErrorInfo(
                ...
                message="无法连接到服务器，请检查网络连接" if is_zh else "Cannot connect to server, please check network connection",
                suggestion="• 检查网络连接是否正常\n• 检查代理设置\n• 确认服务器地址正确"
                if is_zh
                else "• Check network connection\n• Verify proxy settings\n• Confirm server address is correct",
            ...
            "timeout": ErrorInfo(
                ...
                message="服务器响应超时，请稍后重试" if is_zh else "Server response timeout, please retry later",
                suggestion="• 检查网络速度\n• 尝试使用代理\n• 稍后重试"
                if is_zh
            ...
            "proxy": ErrorInfo(
                ...
                message="无法通过代理连接，请检查代理设置" if is_zh else "Cannot connect through proxy, please check proxy settings",
                suggestion="• 检查代理地址格式\n• 确认代理服务可用\n• 尝试关闭代理"
                if is_zh
                else "• Check proxy address format\n• Confirm proxy service is available\n• Try disabling proxy",
                action_button="去设置" if is_zh else "Go to Settings",
            ),
            # API Key errors
            "api_key": ErrorInfo(
                ...
                message="API Key 无效或已过期，请在设置中检查配置" if is_zh else "API Key is invalid or expired, please check configuration in settings",
                suggestion="• 检查 API Key 是否正确\n• 确认 API Key 未过期\n• 检查账户余额"
                if is_zh
                else "• Verify API Key is correct\n• Confirm API Key is not expired\n• Check account balance",
                action_button="去设置" if is_zh else "Go to Settings",
            ),
            "unauthorized": ErrorInfo(
                ...
                message="API 认证失败，请检查 API Key 配置" if is_zh else "API authentication failed, please check API Key configuration",
            ...
            "file_format": ErrorInfo(
                ...
                message="不支持的文件格式，请选择 PDF、Word、PPT 或图片" if is_zh else "Unsupported file format, please select PDF, Word, PPT or images",
                suggestion="• 支持的格式：PDF、DOCX、PPTX、PNG、JPG\n• 检查文件是否损坏\n• 尝试转换为支持的格式"
                if is_zh
                else "• Supported formats: PDF, DOCX, PPTX, PNG, JPG\n• Check if file is corrupted\n• Try converting to supported format",
            ),
            "file_corrupted": ErrorInfo(
                category=ErrorCategory.FILE_FORMAT,
                level=ErrorLevel.ERROR,
                title="文件损坏" if is_zh else "File Corrupted",
                message="文件可能已损坏，无法读取" if is_zh else "File may be corrupted and cannot be read",
                suggestion="• 尝试重新下载文件\n• 使用其他工具打开验证\n• 选择其他文件"
                if is_zh
                else "• Try re-downloading the file\n• Verify with other tools\n• Select another file",
            ),
            "file_too_large": ErrorInfo(
                category=ErrorCategory.FILE_FORMAT,
                level=ErrorLevel.WARNING,
                title="文件过大" if is_zh else "File Too Large",
                message="文件大小超过限制，可能导致处理缓慢" if is_zh else "File size exceeds limit, may cause slow processing",
                suggestion="• 尝试分割文件\n• 压缩文件大小\n• 启用自动分割功能"
                if is_zh
                else "• Try splitting the file\n• Compress file size\n• Enable auto-split feature",
                action_button="去设置" if is_zh else "Go to Settings",
            ),
            # OCR errors
            "ocr": ErrorInfo(
                ...
                message="OCR 识别失败，请确保图片清晰" if is_zh else "OCR recognition failed, please ensure image is clear",
                suggestion="• 使用更清晰的图片\n• 确保文字可读\n• 尝试调整图片亮度/对比度"
                if is_zh
                else "• Use clearer images\n• Ensure text is readable\n• Try adjusting image brightness/contrast",
                action_button="重试" if is_zh else "Retry",
            ),
            "ocr_model": ErrorInfo(
                category=ErrorCategory.OCR,
                ...
                message="OCR 模型文件缺失，需要下载" if is_zh else "OCR model files are missing and need to be downloaded",
                suggestion="• 点击下载按钮获取模型\n• 检查网络连接\n• 确保有足够磁盘空间"
                if is_zh
                else "• Click download button to get models\n• Check network connection\n• Ensure sufficient disk space",
            ...
            "anki_connection": ErrorInfo(
                ...
                message="无法连接到 AnkiConnect，请确保 Anki 正在运行" if is_zh else "Cannot connect to AnkiConnect, please ensure Anki is running",
                suggestion="• 启动 Anki 桌面应用\n• 安装 AnkiConnect 插件\n• 检查 AnkiConnect 设置"
                if is_zh
                else "• Start Anki desktop application\n• Install AnkiConnect add-on\n• Check AnkiConnect settings",
                action_button="去设置" if is_zh else "Go to Settings",
            ),
            "anki_permission": ErrorInfo(
                ...
                message="AnkiConnect 拒绝访问，请检查权限设置" if is_zh else "AnkiConnect denied access, please check permission settings",
                suggestion="• 检查 AnkiConnect 配置\n• 确认 API Key 正确\n• 重启 Anki"
                if is_zh
                else "• Check AnkiConnect configuration\n• Confirm API Key is correct\n• Restart Anki",
                action_button="去设置" if is_zh else "Go to Settings",
            ),
            # LLM provider errors
            "llm_provider": ErrorInfo(
                ...
                message="LLM 服务调用失败，请检查配置" if is_zh else "LLM service call failed, please check configuration",
                suggestion="• 检查提供商配置\n• 确认 API Key 有效\n• 检查账户额度"
                if is_zh
            ...
            "rate_limit": ErrorInfo(
                category=ErrorCategory.LLM_PROVIDER,
                level=ErrorLevel.WARNING,
                title="请求频率限制" if is_zh else "Rate Limit Exceeded",
                message="请求过于频繁，请稍后重试" if is_zh else "Too many requests, please retry later",
                suggestion="• 等待一段时间后重试\n• 降低并发请求数\n• 升级 API 套餐"
                if is_zh
                else "• Wait a moment and retry\n• Reduce concurrent requests\n• Upgrade API plan",
                action_button="重试" if is_zh else "Retry",
            ),
            "quota_exceeded": ErrorInfo(
                ...
                message="API 配额已用尽，请充值或更换提供商" if is_zh else "API quota exhausted, please recharge or switch provider",
                suggestion="• 充值账户\n• 更换其他提供商\n• 检查账户状态"
                if is_zh
                else "• Recharge account\n• Switch to another provider\n• Check account status",
                action_button="去设置" if is_zh else "Go to Settings",
            ),
            # Permission errors
            "permission": ErrorInfo(
                ...
                message="没有足够的权限执行此操作" if is_zh else "Insufficient permissions to perform this operation",
                suggestion="• 以管理员身份运行\n• 检查文件/文件夹权限\n• 更改保存位置"
                if is_zh
    ...
    def classify_error(self, error: Exception | str) -> ErrorInfo:
        ...
        if any(keyword in error_str for keyword in ["api key", "api_key", "unauthorized", "401", "认证", "密钥"]):
        ...
        return ErrorInfo(
            ...
            suggestion="• 查看详细错误信息\n• 尝试重启应用\n• 联系技术支持"
            if is_zh
            else "• Check detailed error message\n• Try restarting application\n• Contact technical support",
    ...
    def _show_messagebox(self, parent: QWidget, error_info: ErrorInfo) -> None:
        ...
        if error_info.technical_details:
            full_message += f"\n\n技术详情：\n{error_info.technical_details}" if self.language == "zh" else f"\n\nTechnical Details:\n{error_info.technical_details}"
